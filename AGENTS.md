...
- **Android 模组重载 OOM 与"特殊加载"**：模组能否加载取决于 ini 表达式密度（`select(`/`memory.`/`[decal_]`/`copyFrom`）而非文件体积——图片音频解码后落在 native 堆，不占 ART 堆（largeHeap=512MB）配额；进程内重载新旧单位表并存，峰值约翻倍，脚本密集型模组必撞 OOM。Mods 页点「重载」前会用 `ModHeavinessScanner`（`rwpp-core-api` 的 `game/mod/`，含标定样本与阈值）评估启用模组，命中高风险时弹 `SpecialLoadDialog` 引导**重启式加载**：`modPersistStates()` 落盘开关 + `ConfigIO` 写一次性 `pendingEnabledStates`（冷启动扫描由 `ModManagerInject` 回退消费）→ `AppContext.restart()`（Android 走 `scheduleAppRestart`）。OOM 拦截弹窗（`MemoryExhaustedDialog`）也带「立即重启加载」按钮。桌面端堆充足不启用该拦截（`restart()` 返回 false）。
